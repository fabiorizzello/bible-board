...
- Don't render the section at all if there are zero annotations of any kind.
  ...
  - Verify: npx tsc --noEmit && npx vitest run
- [ ] **T03: Wire soft delete with 30s toast undo and Toast.Provider** — Wire the "Elimina" dropdown item in ActionToolbar to soft-delete the selected element: remove it from the visible list, show a HeroUI toast with 30-second timeout and "Annulla" action button, and restore the element on undo or finalize on expiry.
