...
- **Global scope**: `/home/fabio/dev/board/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/fabio/dev/board/.claude/notes/`)
...
2. Glob `/home/fabio/dev/board/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/fabio/dev/board/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/fabio/dev/board/.claude/notes/` — works in any directory
