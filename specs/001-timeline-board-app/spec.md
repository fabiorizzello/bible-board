# Feature Specification: Timeline Board App

**Feature Branch**: `001-timeline-board-app`
**Created**: 2026-03-23
**Status**: Draft
**Input**: Applicazione PWA per lo studio di timeline bibliche e storiche con workspace collaborativi, elementi tipizzati, board multi-vista, e sincronizzazione offline via Jazz CRDTs.
...
L'utente aggiunge fonti informative agli Elementi. Tre tipi: "scrittura" (riferimento biblico con link calcolato verso wol.jw.org), "articolo" (URL diretto a wol.jw.org), "altro" (descrizione libera). Le fonti documentano da dove proviene l'informazione.
...
**Independent Test**: L'utente aggiunge fonti di tutti e 3 i tipi a un Elemento, verifica che i link wol.jw.org funzionino correttamente, e che le fonti siano visibili nel dettaglio.

**Acceptance Scenarios**:

1. **Given** un Elemento, **When** l'utente aggiunge una fonte di tipo "scrittura" con riferimento "Genesi 12:1-3", **Then** la fonte viene salvata e il sistema calcola il link verso wol.jw.org.
2. **Given** un Elemento con fonte "scrittura", **When** l'utente tocca la fonte, **Then** il sistema apre il link calcolato verso wol.jw.org con il versetto corretto.
3. **Given** un Elemento, **When** l'utente aggiunge una fonte di tipo "articolo" con URL wol.jw.org, **Then** la fonte viene salvata con l'URL diretto.
4. **Given** un Elemento con fonte "articolo", **When** l'utente tocca la fonte, **Then** il sistema apre il link wol.jw.org salvato.
5. **Given** un Elemento, **When** l'utente aggiunge una fonte di tipo "altro" con descrizione "Enciclopedia biblica, vol. 2, p. 341", **Then** la fonte viene salvata come testo libero.

---

### US-07 - Media Immagini Offline (Priority: P2)

L'utente aggiunge immagini agli Elementi. Le immagini vengono salvate offline come blob nel dispositivo e sono visibili anche senza connessione. L'utente puo visualizzare lo spazio occupato e rimuovere immagini selettivamente.

**Why this priority**: Le immagini arricchiscono lo studio (mappe, illustrazioni, diagrammi) ma non sono bloccanti per il flusso base.
...
- **FR-002**: Il sistema DEVE supportare 7 tipi di Elemento: personaggio, guerra, profezia, regno, periodo, luogo, evento — ciascuno con attributi specifici del tipo.
...
- **FR-006**: Le fonti DEVONO supportare 3 tipi: "scrittura" (riferimento biblico, link calcolato a wol.jw.org), "articolo" (URL diretto a wol.jw.org), "altro" (descrizione libera). Per le fonti "scrittura" il sistema DEVE salvare il riferimento e calcolare il link, senza salvare offline il testo biblico. Per le fonti "articolo" il sistema DEVE salvare solo l'URL, senza salvare offline il contenuto dell'articolo.
- **FR-007**: I media DEVONO supportare immagini salvate offline come blob nel dispositivo. I video NON sono supportati in questo scope.
...
- **Elemento**: Entita base del dominio — un fatto, persona, luogo o concetto studiato. Ha un tipo discriminato (personaggio, guerra, profezia, regno, periodo, luogo, evento) con attributi specifici per tipo e al massimo una DataTemporale generale. Per i personaggi possono esistere anche `nascita` e `morte` come DataStorica dedicate. Una profezia con piu adempimenti viene rappresentata come piu Elementi collegati, non come un singolo Elemento con date multiple.
...
- **Fonte**: Sorgente informativa di un Elemento — scrittura biblica (riferimento, link calcolato), articolo (URL wol.jw.org), o altro (descrizione libera). Le fonti salvano riferimenti e URL, non il contenuto offline di scritture o articoli.
...
+-- tags: Tag[]
+-- fonti: Fonte[]
|   +-- scrittura { riferimento }
|   +-- articolo  { url }
|   +-- altro     { descrizione }
+-- media: Media[]
|   +-- immagine { blob, offline }
+-- note: string
+-- link: ElementoLink[]
...
- Q: L'utente puo avere piu workspace? → A: Workspace singolo per utente. Nessuna schermata di selezione workspace.
- Q: La vista del Board e fissa o switchabile? → A: La vista NON e attributo del Board. Ogni Board puo essere visualizzato in qualsiasi vista in qualsiasi momento. Il Board memorizza solo l'ultima vista usata. L'identita del Board e la sua selezione (dinamica con range date/tag/tipi, oppure fissa con elementi scelti manualmente).
- Q: Esiste una funzionalita di ricerca? → A: Si, ricerca full-text adattata alla vista corrente. Il comportamento della ricerca cambia in base al view mode (lista filtra righe, timeline evidenzia/filtra elementi, grafo evidenzia nodi, ecc.).
- Q: Come funziona l'eliminazione di un Elemento? → A: Conferma con preview impatto (mostra quanti link verranno rimossi) + undo via toast dopo l'eliminazione.
- Q: Come viene creato un Board? → A: Due canali. (1) Creazione esplicita: nome + tipo selezione (fissa vuota o dinamica con filtri base), configurazione successiva. (2) Creazione implicita: durante la navigazione/studio focalizzato, l'utente puo salvare il percorso corrente come Board (selezione fissa con gli elementi visitati).
- Q: Come si combinano i filtri di un Board dinamico? → A: AND tra categorie di filtro; OR tra valori della stessa categoria.
- Q: Come funzionano i permessi di collaborazione? → A: Permessi solo a livello workspace, con ruoli lettura o scrittura applicati a tutto il contenuto condiviso.
- Q: Come funziona il rollback nel log azioni? → A: Come nuova azione compensativa; se non e piu sicuro, il rollback viene impedito con motivo esplicito.
- Q: Come funziona la ricerca nel Board? → A: Ricerca testuale semplice case-insensitive; query vuota = nessun filtro; nessun operatore avanzato.
