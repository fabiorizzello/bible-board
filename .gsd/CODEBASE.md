# Codebase Map

Generated: 2026-04-22T12:26:19Z | Files: 70 | Described: 0/70
<!-- gsd:codebase-meta {"generatedAt":"2026-04-22T12:26:19Z","fingerprint":"c28f27c307ae136f812549d21762155730be6b2c","fileCount":70,"truncated":false} -->

### (root)/
- `.gitignore`
- `.mcp.json`
- `AGENTS.md`
- `CLAUDE.md`
- `codex.md`
- `index.html`
- `package-lock.json`
- `package.json`
- `pnpm-lock.yaml`
- `postcss.config.js`
- `tsconfig.app.json`
- `tsconfig.json`
- `vite.config.ts`
- `vitest.config.ts`

### src/
- `src/main.tsx`
- `src/vite-env.d.ts`

### src/app/
- `src/app/auth-context.tsx`
- `src/app/auth-guards.tsx`
- `src/app/pwa.ts`
- `src/app/router.tsx`

### src/features/board/
- `src/features/board/board.adapter.ts`
- `src/features/board/board.errors.ts`
- `src/features/board/board.model.ts`
- `src/features/board/board.rules.ts`
- `src/features/board/board.schema.ts`

### src/features/elemento/
- `src/features/elemento/elemento.adapter.ts`
- `src/features/elemento/elemento.errors.ts`
- `src/features/elemento/elemento.model.ts`
- `src/features/elemento/elemento.rules.ts`
- `src/features/elemento/elemento.schema.ts`
- `src/features/elemento/wol-link-resolver.ts`

### src/features/elemento/__tests__/
- `src/features/elemento/__tests__/elemento.rules.test.ts`

### src/features/shared/
- `src/features/shared/newtypes.ts`
- `src/features/shared/value-objects.ts`

### src/features/workspace/
- `src/features/workspace/workspace.adapter.ts`
- `src/features/workspace/workspace.errors.ts`
- `src/features/workspace/workspace.model.ts`
- `src/features/workspace/workspace.rules.ts`
- `src/features/workspace/workspace.schema.ts`

### src/mock/
- `src/mock/data.ts`
- `src/mock/index.ts`

### src/mock/__tests__/
- `src/mock/__tests__/data.test.ts`

### src/styles/
- `src/styles/index.css`
- `src/styles/tokens.css`

### src/ui/auth/
- `src/ui/auth/DemoAuthPage.tsx`

### src/ui/mockups/
- `src/ui/mockups/_atoms.tsx`
- `src/ui/mockups/.gitkeep`
- `src/ui/mockups/AddFieldFlowMockup.tsx`
- `src/ui/mockups/CommitInteractionMockup.tsx`
- `src/ui/mockups/CompositeVitaMockup.tsx`
- `src/ui/mockups/DetailLayoutMockup.tsx`
- `src/ui/mockups/HeaderEditMockup.tsx`
- `src/ui/mockups/MarkdownDescrizioneMockup.tsx`
- `src/ui/mockups/milkdown-iframe.css`
- `src/ui/mockups/MockupsIndex.tsx`
- `src/ui/mockups/UnifiedEditorMockup.tsx`
- `src/ui/mockups/ValidationUxMockup.tsx`

### src/ui/not-found/
- `src/ui/not-found/NotFoundPage.tsx`

### src/ui/workspace-home/
- `src/ui/workspace-home/DetailPane.tsx`
- `src/ui/workspace-home/display-helpers.ts`
- `src/ui/workspace-home/ElementoEditor.tsx`
- `src/ui/workspace-home/FullscreenOverlay.tsx`
- `src/ui/workspace-home/ListPane.tsx`
- `src/ui/workspace-home/NavSidebar.tsx`
- `src/ui/workspace-home/ThemeSwitcher.tsx`
- `src/ui/workspace-home/workspace-ui-store.ts`
- `src/ui/workspace-home/WorkspacePreviewPage.tsx`

### src/ui/workspace-home/__tests__/
- `src/ui/workspace-home/__tests__/display-helpers.test.ts`
- `src/ui/workspace-home/__tests__/link-helpers.test.ts`

### tests/
- `tests/setup.ts`
