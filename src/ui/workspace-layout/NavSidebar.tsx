...
export function NavSidebar({ open, onClose }: NavSidebarProps) {
  ...
  // Derive active nav key from URL
  const activeNavKey = (() => {
    if (pathname === "/workspace" || pathname === "/workspace/") return "recenti";
    if (pathname.startsWith("/workspace/tutti") || pathname.startsWith("/workspace/elemento/")) return "tutti";
    return "";
  })();

  // Derive active board id from URL
  const activeBoardId = (() => {
    const match = pathname.match(/^\/workspace\/board\/(.+)/);
  ...
  return (
    ...
    >
        ...
        <Dropdown>
          ...
          >
            <Avatar size="sm" className="h-7 w-7 flex-shrink-0">
              <Avatar.Fallback className="bg-primary text-[11px] font-bold text-white">
                W
              </Avatar.Fallback>
            </Avatar>
      ...
      {/* Nav ListBox — Recenti / Tutti gli elementi */}
      <ScrollShadow className="flex-1 overflow-y-auto px-1.5">
        <ListBox
          aria-label="Navigazione principale"
          selectionMode="single"
          disallowEmptySelection
          selectedKeys={activeNavKey ? new Set([activeNavKey]) : new Set()}
          onSelectionChange={(keys) => {
            if (keys === "all" || keys.size === 0) return;
            const key = String([...keys][0]);
            if (key === "recenti") navigate("/workspace");
            else if (key === "tutti") navigate("/workspace/tutti");
          }}
        ...
        <div className="mt-4 mb-1 flex items-center justify-between px-2.5">
          <Text className="text-[10px] font-bold uppercase tracking-wider text-ink-dim">
            Board
          </Text>
          <Tooltip>
            <Button
        ...
        <ListBox
          aria-label="Board"
          selectionMode="single"
          selectedKeys={activeBoardId ? new Set([activeBoardId]) : new Set()}
          onSelectionChange={(keys) => {
            if (keys === "all" || keys.size === 0) return;
            const boardId = String([...keys][0]);
        ...
        >
          {BOARDS.map((board) => (
            <ListBox.Item
              ...
              className="flex items-center gap-2 rounded-lg px-2.5 min-h-[44px] text-[13px] font-medium text-ink-lo cursor-pointer data-[hovered]:bg-primary/6 data-[selected]:bg-accent/10 data-[selected]:text-accent data-[selected]:font-semibold"
            >
